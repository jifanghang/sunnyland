...
export default function AdminManager({ initialItems, userName }: { initialItems: ContentItem[]; userName: string }) {
  ...
  function beginNew(type: ContentType) {
    setFilter(type);
    setEditingId(null);
    setDraft({ ...blankItem, type, imageUrl: type === "product" ? "/catalogue/outdoor/27-inch-aluminum-tennis-racket.png" : "/about.jpg" });
    setMessage("");
  ...
  async function save(event: FormEvent) {
    ...
    if (response.ok) {
      ...
      setEditingId(null);
      setDraft({ ...blankItem, type: filter, imageUrl: filter === "product" ? "/catalogue/outdoor/27-inch-aluminum-tennis-racket.png" : "/about.jpg" });
    } else {
      const error =
